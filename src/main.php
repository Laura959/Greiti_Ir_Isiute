...
<main>
	...
	GROUP BY 1";
		...
		while($row = $result->fetch(PDO::FETCH_ASSOC)){    
			if ($i == $number)
			{
			echo "<tr><td>".$row['Pavadinimas']."</td><td>".$row['Aprasymas']."</td><td>".$row['Busena']."</td><td class='progresss'><p class='progress-numbers'>".$row['Total_tasks']."/".$row['Todo_tasks']."</p><div class='hover-info'>All tasks: ".$row['Total_tasks'].", Finished tasks".($row['Total_tasks'] - $row['Todo_tasks']).", To do tasks: ".$row['Todo_tasks'].".</div><div class='round'>
			<div class='progress-colors'></div>
		  </div></td><td class='td-spacing'><button><i class='far fa-edit'></i></button><button><i class='far fa-trash-alt'></i></button><button><i class='fas fa-archive'></i></button><button><i class='fas fa-arrow-down'></i></button></td><td><button id='create-button'><i class='fas fa-plus-circle' id='plus-button'></i></button></td></tr>";
			break;
			}
			...
			<div class='progress-colors'></div>
		  </div></td><td class='grey-border'><button><i class='far fa-edit'></i></button><button><i class='far fa-trash-alt'></i></button><button><i class='fas fa-archive'></i></button><button><i class='fas fa-arrow-down'></i></button></td></tr>";
			// $count++;
			// if ($count>2) {
